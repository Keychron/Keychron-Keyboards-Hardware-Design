# Keychron K8 - Hardware Design Files

> Tenkeyless wireless mechanical keyboard with Bluetooth 5.1, USB-C wired mode, Mac/Windows support, and optional hot-swappable switches.

**Product page:** [Keychron K8 Tenkeyless Wireless Mechanical Keyboard](https://www.keychron.com/products/keychron-k8-tenkeyless-wireless-mechanical-keyboard)

![Keychron K8](https://cdn.shopify.com/s/files/1/0059/0630/1017/t/5/assets/keychronk8wirelessmechanicalkeyboard1-1646210675142.jpg?v=1646210677)

## Specifications

| Spec | Detail |
|---|---|
| Layout | TKL / 80% / ANSI |
| Number of Keys | 87 |
| Switch Type | Gateron G Pro Mechanical / Keychron Super Mechanical / Keychron Optical |
| Frame Material | ABS / ABS + aluminum frame |
| Keycap Material | ABS |
| Keycap Profile | OEM |
| Backlight | White backlight / RGB backlight / RGB backlight aluminum frame |
| Connectivity | Bluetooth 5.1 / USB-C wired |
| Polling Rate | 1000 Hz (wired) / 90 Hz (Bluetooth) |
| Battery | 4000 mAh rechargeable li-polymer |
| Wireless Working Time | Up to 240 hours (backlight off) |
| Compatible System | macOS / Windows |

## Included Files

| File | Description |
|---|---|
| `K8-Bottom-Case.stp` | Bottom case 3D CAD model |
| `K8-US-Full-Model- 20260427.stp` | US full keyboard 3D CAD model |
| `K8-US-Plate.dwg` | US plate 2D CAD drawing |
| `K8-US-Plate.pdf` | US plate reference drawing |
| `K8-US-Plate.stp` | US plate 3D CAD model |
| `K8-US-Top Case.stp` | US top case 3D CAD model |

## Downloads

> **Download note:** If a download link does not respond, copy the link and paste it directly into your browser's address bar. All links below are hosted on Keychron's official server and are safe to use.

| File | Download |
|---|---|
| `K8-Bottom-Case.stp` | [Download](http://www.keychron.co:34568/keychron-github-assets/Keychron-Keyboards-Hardware-Design/K-Series/K8/K8-Bottom-Case.stp) |
| `K8-US-Full-Model- 20260427.stp` | [Download](http://www.keychron.co:34568/keychron-github-assets/Keychron-Keyboards-Hardware-Design/K-Series/K8/K8-US-Full-Model-%2020260427.stp) |
| `K8-US-Plate.dwg` | [Download](http://www.keychron.co:34568/keychron-github-assets/Keychron-Keyboards-Hardware-Design/K-Series/K8/K8-US-Plate.dwg) |
| `K8-US-Plate.pdf` | [Download](http://www.keychron.co:34568/keychron-github-assets/Keychron-Keyboards-Hardware-Design/K-Series/K8/K8-US-Plate.pdf) |
| `K8-US-Plate.stp` | [Download](http://www.keychron.co:34568/keychron-github-assets/Keychron-Keyboards-Hardware-Design/K-Series/K8/K8-US-Plate.stp) |
| `K8-US-Top Case.stp` | [Download](http://www.keychron.co:34568/keychron-github-assets/Keychron-Keyboards-Hardware-Design/K-Series/K8/K8-US-Top%20Case.stp) |

## Opening the Files

| Format | Free Software | Commercial Software |
|---|---|---|
| `.stp` | FreeCAD, Onshape | Fusion 360, SolidWorks |
| `.dwg` | LibreCAD (limited), QCAD | AutoCAD, DraftSight |
| `.pdf` | Any PDF viewer | - |

## Links

- [Main Repository](https://github.com/Keychron/Keychron-Keyboards-Hardware-Design)

## License

This project is **source-available**. Personal and educational use is allowed, and commercial use is allowed for original compatible accessories within the limits of the license.

**You may not use these files to copy, manufacture, sell, or distribute Keychron keyboards or mice, or substantially similar products, and you may not use Keychron trademarks as your own branding.**

See the [LICENSE](../../LICENSE) for full terms.
