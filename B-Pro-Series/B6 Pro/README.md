# Keychron B6 Pro - Hardware Design Files

> Ultra-slim full-size wireless keyboard with 2.4 GHz, Bluetooth, wired connectivity, and Keychron Launcher customization.

**Product page:** [Keychron B6 Pro Ultra-Slim Wireless Keyboard](https://www.keychron.com/products/keychron-b6-pro-ultra-slim-wireless-keyboard)

![Keychron B6 Pro](http://www.keychron.com/cdn/shop/files/B6-Pro-Iconic-Features.jpg?crop=center&height=1200&v=1761279632&width=1200)

## Specifications

| Spec | Detail |
|---|---|
| Layout | 100% / full size |
| Number of Keys | 109 |
| Body Material | ABS |
| Keycap Material | ABS |
| Backlight | Non-backlit |
| Connectivity | 2.4 GHz / Bluetooth 5.2 / USB-C wired |
| Polling Rate | 1000 Hz (2.4 GHz and wired) / 90 Hz (Bluetooth) |
| Battery | 800 mAh rechargeable li-polymer |
| Wireless Working Time | Up to 8 months |
| Firmware | Keychron Launcher / ZMK |

## Included Files

| File | Description |
|---|---|
| `B6-Pro-JIS-Full-Model-20240920.STEP.zip` | JIS full keyboard 3D model archive |
| `B6-Pro-UK-Full-Model-20240801.STEP.zip` | UK full keyboard 3D model archive |
| `B6-Pro-US-Full-Model.stp.zip` | US full keyboard 3D model archive |
| `B6_PRO_BOTTOM_CASE.stp` | Bottom case 3D CAD model |
| `B6_PRO_UK_TOP_CASE.stp` | UK top case 3D CAD model |
| `B6_PRO_US_TOP_CASE.stp` | US top case 3D CAD model |

## Downloads

> **Download note:** If a download link does not respond, copy the link and paste it directly into your browser's address bar. All links below are hosted on Keychron's official server and are safe to use.

| File | Download |
|---|---|
| `B6-Pro-JIS-Full-Model-20240920.STEP.zip` | [Download](http://www.keychron.co:34568/keychron-github-assets/Keychron-Keyboards-Hardware-Design/B-Pro-Series/B6%20Pro/B6-Pro-JIS-Full-Model-20240920.STEP.zip) |
| `B6-Pro-UK-Full-Model-20240801.STEP.zip` | [Download](http://www.keychron.co:34568/keychron-github-assets/Keychron-Keyboards-Hardware-Design/B-Pro-Series/B6%20Pro/B6-Pro-UK-Full-Model-20240801.STEP.zip) |
| `B6-Pro-US-Full-Model.stp.zip` | [Download](http://www.keychron.co:34568/keychron-github-assets/Keychron-Keyboards-Hardware-Design/B-Pro-Series/B6%20Pro/B6-Pro-US-Full-Model.stp.zip) |
| `B6_PRO_BOTTOM_CASE.stp` | [Download](http://www.keychron.co:34568/keychron-github-assets/Keychron-Keyboards-Hardware-Design/B-Pro-Series/B6%20Pro/B6_PRO_BOTTOM_CASE.stp) |
| `B6_PRO_UK_TOP_CASE.stp` | [Download](http://www.keychron.co:34568/keychron-github-assets/Keychron-Keyboards-Hardware-Design/B-Pro-Series/B6%20Pro/B6_PRO_UK_TOP_CASE.stp) |
| `B6_PRO_US_TOP_CASE.stp` | [Download](http://www.keychron.co:34568/keychron-github-assets/Keychron-Keyboards-Hardware-Design/B-Pro-Series/B6%20Pro/B6_PRO_US_TOP_CASE.stp) |

## Opening the Files

| Format | Free Software | Commercial Software |
|---|---|---|
| `.stp` / `.step` | FreeCAD, Onshape | Fusion 360, SolidWorks |
| `.zip` | Built-in archive utility, 7-Zip | WinZip, BetterZip |

## Links

- [Main Repository](https://github.com/Keychron/Keychron-Keyboards-Hardware-Design)

## License

This project is **source-available**. Personal and educational use is allowed, and commercial use is allowed for original compatible accessories within the limits of the license.

**You may not use these files to copy, manufacture, sell, or distribute Keychron keyboards or mice, or substantially similar products, and you may not use Keychron trademarks as your own branding.**

See the [LICENSE](../../LICENSE) for full terms.
