# Keychron Q65 Max - Hardware Design Files

> 65% full-metal QMK/VIA wireless custom mechanical keyboard with a programmable knob, tri-mode connectivity, and acoustic foams.

**Product page:** [Keychron Q65 Max](https://www.keychron.com/products/keychron-q65-max-qmk-via-wireless-custom-mechanical-keyboard)

![Q65 Max](https://cdn.shopify.com/s/files/1/0059/0630/1017/files/Keychron-Q65-Max-QMK-VIA-Custom-Mechanical-Keyboard-1.jpg)

## Specifications

| Spec | Detail |
|---|---|
| Layout | 65% |
| Body Material | Full CNC aluminum |
| Mount | Double-gasket |
| Knob | Yes |
| Connectivity | 2.4 GHz / Bluetooth 5.1 / USB-C wired |
| Polling Rate | 1000 Hz (2.4 GHz and wired) / 90 Hz (Bluetooth) |
| Switch Support | Hot-swappable 3-pin and 5-pin MX mechanical switches |
| Keycaps | KSA double-shot PBT |
| Firmware | QMK |

## Included Files

| File | Description |
|---|---|
| `Q65_Max-US_KSA_Full_Model_20260421.stp.zip` | US full keyboard 3D model archive with KSA profile keycaps |
| `Q65_Max_US_Bottom_Case.stp` | US bottom case 3D CAD model |
| `Q65_Max_US_PCB 20230801.dwg` | US PCB 2D CAD drawing |
| `Q65_Max_US_Plate.dwg` | US plate 2D CAD drawing |
| `Q65_Max_US_Top_Case.stp` | US top case 3D CAD model |

## Downloads

| File | Download |
|---|---|
| `Q65_Max-US_KSA_Full_Model_20260421.stp.zip` | [Download](http://www.keychron.co:34568/keychron-github-assets/Keychron-Keyboards-Hardware-Design/Q-Max-Series/Q65%20Max/Q65_Max-US_KSA_Full_Model_20260421.stp.zip) |
| `Q65_Max_US_Bottom_Case.stp` | [Download](http://www.keychron.co:34568/keychron-github-assets/Keychron-Keyboards-Hardware-Design/Q-Max-Series/Q65%20Max/Q65_Max_US_Bottom_Case.stp) |
| `Q65_Max_US_PCB 20230801.dwg` | [Download](http://www.keychron.co:34568/keychron-github-assets/Keychron-Keyboards-Hardware-Design/Q-Max-Series/Q65%20Max/Q65_Max_US_PCB%2020230801.dwg) |
| `Q65_Max_US_Plate.dwg` | [Download](http://www.keychron.co:34568/keychron-github-assets/Keychron-Keyboards-Hardware-Design/Q-Max-Series/Q65%20Max/Q65_Max_US_Plate.dwg) |
| `Q65_Max_US_Top_Case.stp` | [Download](http://www.keychron.co:34568/keychron-github-assets/Keychron-Keyboards-Hardware-Design/Q-Max-Series/Q65%20Max/Q65_Max_US_Top_Case.stp) |

## Opening the Files

| Format | Free Software | Commercial Software |
|---|---|---|
| `.stp` | FreeCAD, Onshape | Fusion 360, SolidWorks |
| `.dwg` | LibreCAD (limited), QCAD | AutoCAD, DraftSight |
| `.zip` | Built-in archive utility, 7-Zip | WinZip, BetterZip |

## Links

- [Series Overview](../README.md)
- [Q Max Collection](https://www.keychron.com/collections/keychron-q-max-series-keyboard)
- [Main Repository](https://github.com/Keychron/Keychron-Keyboards-Hardware-Design)

## License

This project is **source-available**. Personal and educational use is allowed, and commercial use is allowed for original compatible accessories within the limits of the license.

**You may not use these files to copy, manufacture, sell, or distribute Keychron keyboards or mice, or substantially similar products, and you may not use Keychron trademarks as your own branding.**

See the [LICENSE](../../LICENSE) for full terms.
